...
- Authenticate to Distr's private registry using a Personal Access Token (PAT)
- Pull the Helm chart and container images from the Distr registry
...
1. **Create image pull secret**
   ...
   Log in to your Distr customer portal and generate a Personal Access Token (PAT), then create a Kubernetes secret:
...
2. **Authenticate Helm with Distr**

   ```bash
   echo "YOUR_DISTR_ACCESS_TOKEN" | helm registry login registry.distr.sh -u - --password-stdin
   ```

3. **Pull the Helm chart**

   ```bash
   helm pull oci://registry.distr.sh/basedash/basedash-charts --version 0.1.0
   ```
...
Pull the new chart version and upgrade:

```bash
helm pull oci://registry.distr.sh/basedash/basedash-charts --version X.Y.Z
