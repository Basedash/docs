...
- The application validates the license at startup; startup fails fast if the license is invalid
- Periodic heartbeats (every 6 hours) are sent to the Basedash license server
...
- If we fail to validate the license on a heartbeat, the app retries with exponential backoff (starting at 5 minutes, doubling up to 6 hours). The app continues to run and shows a warning banner until the next successful heartbeat
...
3. The application continues to run and will retry with exponential backoff (5 minutes, then doubling up to 6 hours) until the next successful heartbeat

**"License has expired" error:**

Contact Basedash at support@basedash.com to renew your license. Expiration takes effect immediately; new organization creation is disabled until you renew.
