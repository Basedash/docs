...
[Alerts](/features/alerts) can be configured to send notifications to Slack channels when they're generated. This keeps your team informed about scheduled alerts without requiring them to check Basedash manually.

To enable Slack notifications for an alert:
...
When the alert runs on its schedule, a notification will be posted to the selected channel with a link to view the full alert.

<Note>
  To send alerts to private channels, you'll need to invite the bot to the
  channel first by typing `/invite @Basedash` in that channel.
</Note>
