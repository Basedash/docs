...
For embedded dashboards that need locked filter values (e.g., filtering by tenant or user), you can use JWT-based secure filtering. This allows you to pre-set filter values that viewers cannot modify. See [secure filtering](/features/filters-and-variables#secure-filtering) for details.
