...
<Note>
  Full app embedding requires API access. Contact support@basedash.com to enable the public API for your account.
</Note>
...
<Warning>
  Store your `embedJwtSecret` securely in your environment variables. Never expose it in client-side code.
</Warning>
...
Never expose the `embedJwtSecret` in client-side code. Generate JWTs only on your backend.
</Accordion>
...
Add your domain to the `embedAllowedOrigins` array. Make sure to include the full origin (e.g., `https://app.example.com`).
</Accordion>

<Accordion title="Authentication failed (invalid or expired JWT)">
**Error**: "The JWT token is invalid or has expired"

- Verify you're using the correct `embedJwtSecret` for the organization
