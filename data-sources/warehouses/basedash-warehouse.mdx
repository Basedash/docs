...
- **Share insights** - Collaborate with team members on [reports](/features/autopilot) and [dashboards](/features/dashboards)
...
4. Set up automated [reports](/features/autopilot)
...
You can access your Basedash Warehouse credentials directly from the app to connect external tools, run custom queries, or manage your data.
...
<Note>
You must have at least one Fivetran connection set up through Basedash Warehouse for this option to be available.
</Note>

### Connecting with external tools

Use your credentials to connect any PostgreSQL-compatible client:
...
Connect to your warehouse and run SQL commands to delete specific records:
...
Execute complex queries that aren't supported through the Basedash interface:

```sql
-- Example: Export data to analyze externally
...
Set up views, functions, or [row-level security policies](/features/row-level-security):

```sql
-- Create a view for filtered data access
CREATE VIEW schema_name.filtered_view AS
SELECT * FROM schema_name.table_name
WHERE status = 'active';
```

<Warning>
Direct database modifications bypass Basedash's data management features. Use caution when deleting or modifying data directly.
</Warning>
...
- **Build [dashboards](/features/dashboards)** - Create visualizations and [reports](/features/autopilot)
- **Set up alerts** - Monitor key metrics and get notified of changes
- **Share insights** - Collaborate with your team on data analysis
- **Automate reporting** - Schedule regular [reports](/features/autopilot) and updates
