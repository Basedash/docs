---
title: "November 28th"
description: "Two-way Slack chat sync, dashboard tabs and ultrawide layouts, chart goal lines and grouped bars, plus security hardening and more reliable scheduled reports."
---
...
We tightened several security surfaces. File uploads are now restricted to safe types, closing off the ability to upload potentially dangerous executables or scripts. Avatar URLs and organization/user text fields are sanitized and validated so that malicious HTML or untrusted external image URLs can't be stored and rendered back in the UI.

Authentication flows are safer and more privacy‑preserving. Login responses no longer reveal whether an email address exists in the system, which prevents username enumeration attacks, and we improved handling of race conditions and error states around user creation and sign‑up edge cases. These changes make the product more resilient without changing the day‑to-day sign‑in experience for legitimate users.
...
- Surface backend errors more clearly by showing a toast whenever a change fails in the background, instead of silently reverting.
